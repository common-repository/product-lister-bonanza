...
?>
<div id="ced_bonanza_lister_fields" class="panel woocommerce_options_panel">
	<div id="ced_bonanza_lister_accordian">
	<!-- --------------------------Required fields-------------------------------- -->
		<?php if(count($required_fields)) : ?>
		<div class="ced_bonanza_lister_panel">
			<div class="ced_bonanza_lister_panel_heading">
				<h4><?php _e('Required Fields','ced-bonanza') ?></h4>
			</div>
			<div class="ced_bonanza_lister_collapse">
				<div class="options_group ced_bonanza_lister_label_data">
				<?php 

				$requiredInAnyCase = array('_bnz_bonanza_id_type','_bnz_bonanza_id_val','_bnz_bonanza_brand');
							
				if( $isVariableProductCase ) {  
				// adding variation title field only in case of variation product
					$addedFieldForVariableProTitle = array(
						'type' => '_text_input',
						'id'   => '_bnz_bonanza_variation_title',
						'fields' => array(
							'id'   => '_bnz_bonanza_variation_title',
							'label' => __('Variation Title', 'ced-bonanza'),
							'desc_tip' => '',
							'description' => __('This Will Be Your Variation Name In Your MarketPlace.','ced-bonanza')
						)	
					);
				...
				foreach ($required_fields as $field_array):
					if(isset($field_array['id']) && isset($field_array['type']) && isset($field_array['fields'])){
						...
						if($type=='_bnz_bonanza_select'){
							...
							$optionsHtml = '';
							$optionsHtml .= '<option value="null">'.__('bonanza Subcategory','ced-bonanza').'</option>';
							if(is_array($options)){
							...
							if( $isVariableProductCase ) {
								$this->bnz_render_variation_html($field_array,$loop,$variation);
							}
							else {
	...
	<!-- --------------------------Framework Specific fields-------------------------------- -->	
		...
		<div class="ced_bonanza_lister_panel">
			<div class="ced_bonanza_lister_panel_heading">
				<h4><?php _e('Framework Specific Fields','ced-bonanza'); ?></h4>
			</div>
			<div class="ced_bonanza_lister_collapse">
			<?php foreach($framework_fields as $fname=> $ffields_details): ?>
				<?php if(count($ffields_details)) :?>
				<div class="ced_bonanza_lister_sub_accordion">
					<div class="ced_bonanza_lister_sub_panel">
						<div class="ced_bonanza_lister_sub_panel_heading">	
							<h4><?php echo esc_attr($fname); ?></h4>
						</div>
						<div class="ced_bonanza_lister_sub_collapse">
						<?php 
							foreach($ffields_details as $ffields_array){
								if(isset($ffields_array['id']) && isset($ffields_array['type']) && isset($ffields_array['fields'])){
									...
									if( $isVariableProductCase ) {
										$this->bnz_render_variation_html($ffields_array,$loop,$variation);
									}
									else {
		...
		<div class="ced_bonanza_lister_panel">
			<div class="ced_bonanza_lister_panel_heading">
				<h4><?php _e('Extra Fields','ced-bonanza') ?></h4>
			</div>
			<div class="ced_bonanza_lister_collapse">
				<div class="options_group ced_bonanza_lister_label_data">
					<?php 
					foreach($extra_fields as $efield_array):
						if(isset($efield_array['id']) && isset($efield_array['type']) && isset($efield_array['fields'])){
							...
							if( $isVariableProductCase ) {
								$this->bnz_render_variation_html($efield_array,$loop,$variation);
							}
							else {
									...
									switch($etype){
										case 'lwh' :
											...
											?><p class="form-field dimensions_field">
												...
												<span class="wrap">
													<input id="<?php $id; ?>" placeholder="<?php esc_attr_e( 'Length', 'ced-bonanza' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="<?php echo $id ?>_length" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_length', true ) ) ); ?>" />
													<input placeholder="<?php esc_attr_e( 'Width', 'ced-bonanza' ); ?>" class="input-text wc_input_decimal" size="6" type="text" name="<?php echo $id ?>_width" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_width', true ) ) ); ?>" />
													<input placeholder="<?php esc_attr_e( 'Height', 'ced-bonanza' ); ?>" class="input-text wc_input_decimal last" size="6" type="text" name="<?php echo  $id ?>_height" value="<?php echo esc_attr( wc_format_localized_decimal( get_post_meta( $post->ID, $id.'_height', true ) ) ); ?>" />
												</span>
